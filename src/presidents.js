...
let democratPresidents = []
// Iteration 2 | Democratic Presidents - `filter()`
function getDemocraticPresidents (presidentsArr) {
  return presidentsArr.filter(item => item.party === 'Democratic')
}
...
function countYearsInOffice (presidentsArr) {
  return presidentsArr.reduce((acc, cu) => {
    if (cu.leftOffice === null) {
      return acc
    } else {
      let totalAge = cu.leftOffice - cu.tookOffice
      return acc + totalAge
    }
  }, 0)
}
...
function getAgeAtInauguration (presidentsArr) {
  // return presidentsArr.map(item => {
  //   return { ageAtInauguration: item.tookOffice - item.birthYear }
  // })
  return presidentsArr.map(item => {
    item.ageAtInauguration = item.tookOffice - item.birthYear
    return item
  })
}
